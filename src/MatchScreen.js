...
import {Pressable, Text, View,StyleSheet,TouchableOpacity, SafeAreaView} from 'react-native';
...
const DisplayScreen = ({sub}) => {
 ...
 const handleLike = async () => {
  ...
  const handleDislike = async () => {
   console.log("dislike")
   await DataStore.delete(WaitlingList, (u) => u.user1('eq', currentUser.sub).user2('eq',users[index].sub));
   await DataStore.delete(Matches, (u) => u.user1('eq', currentUser.sub).user2('eq',users[index].sub));
   await DataStore.delete(Matches, (u) => u.user2('eq', currentUser.sub).user1('eq',users[index].sub));
   setIndex((index+1)%users.length)
 }
  return(
